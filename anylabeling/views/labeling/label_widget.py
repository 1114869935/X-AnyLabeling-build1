...
class LabelingWidget(LabelDialog):
    ...
    def __init__(  # noqa: C901
        ...
        if hasattr(self, 'canvas'):
            self.canvas.selection_changed.connect(self.update_split_button_state)

        # B. 连接我们新按钮的“点击”信号
        if hasattr(self, 'auto_labeling_widget') and hasattr(self.auto_labeling_widget, 'button_split_mask'):
            self.auto_labeling_widget.button_split_mask.clicked.connect(self.start_split_mode)

        # C. 连接“模型切换”信号
        if hasattr(self, 'auto_labeling_widget'):
            self.auto_labeling_widget.model_manager.model_loaded.connect(self.update_split_button_state)

        # D. 连接 Canvas 画线完成的信号
        if hasattr(self, 'canvas'):
            self.canvas.lineDrawnForSplit.connect(self.on_line_drawn_for_split)
    ...
    def update_split_button_state(self):
        """
        控制“分割 Mask”按钮的启用/禁用状态。
        """
        if not hasattr(self, 'auto_labeling_widget') or not hasattr(self.auto_labeling_widget, 'button_split_mask'):
            return

        # 【最终正确版本】: 我们获取的是 loaded_model_config
        current_config = self.auto_labeling_widget.model_manager.loaded_model_config

        # 条件1: 我们的模型是否被激活？
        is_our_model_active = (
            current_config is not None and
            current_config.get("type") == "bacteria_autoseg"
        )
        
        # 条件2: 是否选中了一个可被分割的图形？
        selected_shapes = self.canvas.selected_shapes
        is_single_splittable_shape_selected = (
            len(selected_shapes) == 1 and 
            selected_shapes[0].shape_type in ["polygon", "rectangle", "rotation"]
        )

        # 最终决定按钮是否可用
        enable = is_our_model_active and is_single_splittable_shape_selected
        self.auto_labeling_widget.button_split_mask.setEnabled(enable)

    def start_split_mode(self):
        """
        当用户点击“分割 Mask”按钮时触发此方法。
        这个方法将命令 Canvas 进入“画线分割”模式。
        """
        print("Split Mask button clicked! Entering split mode...")
        
        # 安全检查：确保确实有一个选中的图形
        if not self.canvas.selected_shapes or len(self.canvas.selected_shapes) != 1:
            return
            
        shape_to_split = self.canvas.selected_shapes[0]
    
        # 命令 Canvas 进入新模式，并把要分割的图形告诉它
        self.canvas.enter_split_mode(shape_to_split)
        
        # 在状态栏给用户提示
        self.statusBar().showMessage(
            self.tr("Draw a line to split the selected shape. Press ESC to cancel.")
        )
    ...
    def on_line_drawn_for_split(self, shape_to_split, line_points):
        """
        用用户画的线将选中的 polygon Mask 切成两份（纯本地几何法，不调用模型）。
        新增：若划线首尾相连（闭合圈），可把圈内与圈外（仍在原形状内）切开。
        其他：自动外延分割线；差集切割带 & polygonize 兜底；QPointF；0参信号 emit；UI 同步。
        """
        print("Line drawn! Splitting locally without model...")
    
        # ---------- 依赖 ----------
        try:
            from shapely.geometry import Polygon, LineString, LinearRing, MultiPolygon
            from shapely.ops import split as shapely_split, polygonize, linemerge, unary_union
        except ImportError:
            self.statusBar().showMessage(self.tr("Please install shapely: pip install shapely"), 5000)
            return
        ...
        def _poly_to_points(shapely_poly):
            coords = list(shapely_poly.exterior.coords)
            if len(coords) > 1 and coords[0] == coords[-1]:
                coords = coords[:-1]
            return [QPointF(float(x), float(y)) for x, y in coords]
    
        def _extend_linestring(line: "LineString", bounds, factor=0.5):
            minx, miny, maxx, maxy = bounds
            span = max(maxx - minx, maxy - miny)
            pad = max(4.0, span * factor)
            bb = Polygon([
            ...
            def _unit(a, b):
                ...
                l = (vx * vx + vy * vy) ** 0.5
                return (1.0, 0.0) if l == 0 else (vx / l, vy / l)
    
            v0 = _unit(p0, p1)
            v1 = _unit(q0, q1)
            ext_len = max(200.0, span * 2.5)
            new_start = (p0[0] + v0[0] * ext_len, p0[1] + v0[1] * ext_len)
            new_end   = (q0[0] + v1[0] * ext_len, q0[1] + v1[1] * ext_len)
            ext_line = LineString([new_start] + pts + [new_end]).intersection(bb)
            try:
                ext_line = linemerge(ext_line)
            except Exception:
                pass
            return ext_line
    
        def _get_shape_list():
            for name in ("shapes", "_shapes", "items", "_items"):
                lst = getattr(self.canvas, name, None)
                if isinstance(lst, list):
                    return lst
            return None
    
        def _refresh_canvas():
            for name in ("update", "repaint", "viewportUpdate"):
        ...
        def _is_closed_line(xy, eps=1e-6):
            """判断用户画的线是否首尾相连（闭合）"""
            if len(xy) < 3:
                return False
            x0, y0 = xy[0]
            x1, y1 = xy[-1]
            dx, dy = x0 - x1, y0 - y1
            return (dx * dx + dy * dy) ** 0.5 <= eps
    
        # ---------- 输入检查 ----------
        poly_xy = _to_xy_list(getattr(shape_to_split, "points", []))
        line_xy = _to_xy_list(line_points)
    
        if len(poly_xy) < 3:
            self.statusBar().showMessage(self.tr("Selected shape is not a valid polygon."), 3000)
            return
        if len(line_xy) < 2:
            self.statusBar().showMessage(self.tr("The splitting line is too short."), 3000)
            return
        ...
        raw_line = LineString(line_xy)
        if raw_line.length == 0:
            self.statusBar().showMessage(self.tr("The splitting line length is zero."), 3000)
            return
        ...
        # A) 新增：闭合线分支（小圈/闭合折线）
        if raw_line.is_ring or _is_closed_line(line_xy, eps=1e-6):
            # 确保首尾闭合
            if line_xy[0] != line_xy[-1]:
                ring_xy = line_xy + [line_xy[0]]
            else:
                ring_xy = line_xy
            ring = LinearRing(ring_xy)
    
            # 仅在环与原多边形有交集时处理
            if Polygon(ring).intersects(poly) or LineString(ring_xy).intersects(poly):
                try:
                    merged = unary_union([poly.boundary, ring])
                    faces = list(polygonize(merged))
                    inside = []
                    for g in faces:
                        gi = g.intersection(poly)  # 仅保留落在原形状内部的面
                        if gi.geom_type == "Polygon" and gi.area > 1.0:
                            inside.append(gi)
                    if len(inside) >= 2:
                        class _Wrap:  # 统一接口
                            def __init__(self, geoms): self.geoms = geoms
                        parts = _Wrap(inside)
                        print(f"[closed-loop] polygonize success; parts={len(inside)}")
                except Exception as e:
                    print(f"[closed-loop] exception: {e}")
    
            # 若闭合分支没拿到两块，继续走非闭合常规流程兜底
        # B) 常规：非闭合线（或闭合失败兜底）
        if parts is None or len(list(getattr(parts, "geoms", []))) < 2:
            # 先外延，使之穿越边界
            ext_line = _extend_linestring(raw_line, poly.bounds, factor=0.5)
            print("[extend] line extended.")
    
            try:
                parts_try = shapely_split(poly, ext_line)
                geoms = list(getattr(parts_try, "geoms", []))
                if len(geoms) >= 2:
                    parts = parts_try
                    print(f"[split] success with extended line; parts={len(geoms)}")
            except Exception as e:
            ...
            if parts is None or len(list(getattr(parts, "geoms", []))) < 2:
                ...
                span = max(maxx - minx, maxy - miny)
                widths = [max(1.0, span * r) for r in (0.01, 0.02, 0.03, 0.05, 0.08)]
                print(f"[diff] trying widths: {widths} (span={span})")
                for w in widths:
                    band = ext_line.buffer(w, cap_style=2, join_style=2)
                    diff = poly.difference(band)
                    if isinstance(diff, Polygon):
                        diff = MultiPolygon([diff])
                    cands = [g for g in getattr(diff, "geoms", [])
                             if getattr(g, "geom_type", "") == "Polygon" and g.area > 1.0]
                    print(f"[diff] width={w:.3f}, candidate parts={len(cands)}")
                    if len(cands) >= 2:
                        class _Wrap:
                            def __init__(self, geoms): self.geoms = geoms
                        parts = _Wrap(cands)
            ...
            if parts is None or len(list(getattr(parts, "geoms", []))) < 2:
                try:
                    merged = unary_union([ext_line, poly.boundary])
                    faces = list(polygonize(merged))
                    inside = []
                    for g in faces:
                        gi = g.intersection(poly)
                        if gi.geom_type == "Polygon" and gi.area > 1.0:
                            inside.append(gi)
                    if len(inside) >= 2:
                        class _Wrap:
                            def __init__(self, geoms): self.geoms = geoms
                        parts = _Wrap(inside)
                        print(f"[polygonize] success; parts={len(inside)}")
                except Exception as e:
                    print(f"[polygonize] exception: {e}")
    
        if parts is None or len(list(getattr(parts, "geoms", []))) < 2:
            self.statusBar().showMessage(self.tr("Split failed. Try drawing a full line or a closed loop."), 4000)
            print("[result] not split into >=2 parts")
            return
        ...
        for geom in parts.geoms:
            if getattr(geom, "geom_type", "") != "Polygon" or geom.area <= 1.0:
                continue
            ns = deepcopy(shape_to_split)
            ns.points = _poly_to_points(geom)  # -> list[QPointF]
            # 保险：若外部把点变成了 tuple，再矫正回 QPointF
            for i, pt in enumerate(ns.points):
                if not hasattr(pt, "x"):
                    x, y = pt
                    ns.points[i] = QPointF(float(x), float(y))
            if hasattr(ns, "shape_type"):
                ns.shape_type = "polygon"
            if hasattr(ns, "close"):
                try:
                    ns.close()
                except Exception:
        ...
        if len(new_shapes) < 2:
            self.statusBar().showMessage(self.tr("Split produced less than two valid parts."), 3000)
            print(f"[result] valid new_shapes={len(new_shapes)}")
            return
    
        # ---------- 追加到 canvas.shapes，并触发 0 参信号 ----------
        lst = _get_shape_list()
        if lst is None:
            print("[canvas] no shapes list found; cannot append directly.")
            self.statusBar().showMessage(self.tr("Internal error: shapes list not found."), 4000)
            return
    
        for s in new_shapes[:2]:
            lst.append(s)
            try:
                self.canvas.new_shape.emit()  # 你的项目里该信号为 0 参，用 shapes[-1] 读取
            except Exception as e:
                print(f"[signal] new_shape emit failed: {e}")
    
        # ---------- 删除旧 shape，并从标签列表移除 ----------
        try:
            self.canvas.delete_shape(shape_to_split)
        except Exception as e:
            print(f"[canvas] delete_shape error: {e}")
            try:
                lst.remove(shape_to_split)
            except Exception:
                pass
    
        try:
            self.remove_labels([shape_to_split])
        except Exception as e:
            print(f"[labels] remove_labels error: {e}")
        ...
        _refresh_canvas()
        self.statusBar().showMessage(self.tr("Split successful!"), 3000)
        print(f"[result] done. created={len(new_shapes[:2])}")
